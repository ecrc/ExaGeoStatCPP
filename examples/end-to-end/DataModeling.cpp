...
int main(int argc, char **argv) {
    ...
    data.GetDescriptorData()->SetDescriptor(CHAMELEON_DESCRIPTOR, DESCRIPTOR_Z, is_OOC, z_matrix, float_point, dts, dts,
    ...
    data.GetDescriptorData()->SetDescriptor(CHAMELEON_DESCRIPTOR, DESCRIPTOR_C, is_OOC, c_matrix, float_point, dts, dts,
                                            dts * dts, N, N, 0, 0, N, N, p_grid, q_grid);

    //Initiating the descriptors needed by data modeling.
    data.GetDescriptorData()->SetDescriptor(CHAMELEON_DESCRIPTOR, DESCRIPTOR_Z_COPY, is_OOC, nullptr, float_point, dts,
                                            dts, dts * dts, N, 1, 0, 0, N, 1, p_grid, q_grid);
    data.GetDescriptorData()->SetDescriptor(CHAMELEON_DESCRIPTOR, DESCRIPTOR_PRODUCT, is_OOC, nullptr,
                                            float_point, dts, dts, dts * dts, 1, 1, 0, 0, 1, 1, p_grid, q_grid);
    data.GetDescriptorData()->SetDescriptor(CHAMELEON_DESCRIPTOR, DESCRIPTOR_DETERMINANT, is_OOC, nullptr, float_point,
