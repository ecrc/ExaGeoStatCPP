...
int ForwardSHT(parsec_context_t *apContext, parsec_tiled_matrix_t *apFDataDesc, parsec_tiled_matrix_t *apFLMDesc,
               ...
               int aET2M, int aPN, int aFlmM, int aFlmN, int aLSize, int aNodes, void *apWorkSpace);
 ...
 * @details This function creates a taskpool for determinant calculation, adds it to the PaRSEC context,
 * starts the execution, waits for completion, and finally destructs the taskpool.
...
int ParsecGetZObs(parsec_context_t *parsec, parsec_tiled_matrix_t *dcA, double *Z, int N) ;
 ...
 */
int LapackToTile(parsec_context_t *apParsec, parsec_tiled_matrix_t *apDcA, double *aZobs, int anZobs);
...
/**
 * @brief Execute the MSE Calculation task.
 * @param [in] apParsec Pointer to the PaRSEC context.
 * @param [in,out] apDcZPre  Pointer to the distributed ZPre matrix descriptor.
 * @param [in,out] apDcZMiss Pointer to the distributed ZMiss matrix descriptor.
 * @param [in,out] apDcZError Pointer to the distributed ZError matrix descriptor.
 * @return int Returns 0 upon successful execution.
 */
int MSECalculation(parsec_context_t *apParsec, parsec_tiled_matrix_t *apDcZPre, parsec_tiled_matrix_t *apDcZMiss, parsec_tiled_matrix_t *apDcZError);