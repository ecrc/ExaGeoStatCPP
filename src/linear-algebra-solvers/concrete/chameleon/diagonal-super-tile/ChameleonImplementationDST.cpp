...
ChameleonImplementationDST<T>::ExaGeoStatPotrfTile(const common::UpperLower &aUpperLower, void *apA, int aDiagThick,
    ...
    ExaGeoStatPotrfTileAsync(aUpperLower, apA, aDiagThick, sequence, &request);
...
template<typename T>
int
ChameleonImplementationDST<T>::ExaGeoStatPotrfTileAsync(const common::UpperLower &aUpperLower, void *apA,
                                                        int aDiagThick, void *apSequence, void *apRequest) {
    CHAM_context_t *chamctxt;
...
ChameleonImplementationDST<T>::ExaGeoStatParallelPotrfDiag(const common::UpperLower &aUpperLower, void *apA, int aDiagThick,
    ...
    if (aUpperLower == EXAGEOSTAT_LOWER) {
        for (k = 0; k < A->mt; k++) {
            ...
            for (m = k + 1; m < A->mt && m < k + aDiagThick; m++) {
            ...
            for (n = k + 1; n < A->nt && n < k + aDiagThick; n++) {
                ...
                for (m = n + 1; m < A->mt && m < n + aDiagThick; m++) {
                    tempmm = m == A->mt - 1 ? A->m - m * A->mb : A->mb;
                    ldam = BLKLDD(A, m);
