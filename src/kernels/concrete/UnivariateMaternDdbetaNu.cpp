        ...
        for (j = 0; j < aColumnsNumber; j++) {
            ...
            if (expr == 0) {
                ...
                nu_expr_prime = (1 - apLocalTheta[2]) * 1 / pow(2, apLocalTheta[2]) * 1 / tgamma(apLocalTheta[2])
                                * pow(expr, apLocalTheta[2]) * (apLocalTheta[2] / expr * gsl_sf_bessel_Knu(apLocalTheta[2], expr) - gsl_sf_bessel_Knu(apLocalTheta[2] + 1, expr)) +
                                pow(2, 1 - apLocalTheta[2])
                                * (-1 / tgamma(apLocalTheta[2]) * gsl_sf_psi(apLocalTheta[2]) * pow(expr, apLocalTheta[2])
                                   * (apLocalTheta[2] / expr * gsl_sf_bessel_Knu(apLocalTheta[2], expr) - gsl_sf_bessel_Knu(apLocalTheta[2] + 1, expr)) + 1 / tgamma(apLocalTheta[2])
                                                                            * (pow(expr, apLocalTheta[2]) * log(expr)
                                                                               * (apLocalTheta[2] / expr * gsl_sf_bessel_Knu(apLocalTheta[2], expr) - gsl_sf_bessel_Knu(apLocalTheta[2] + 1, expr)) +
                                                                               pow(expr, apLocalTheta[2])
                                                                               * CalculateSecondDerivativeBesselNuInput(apLocalTheta[2], expr)));

                apMatrixA[i + j * aRowsNumber] = (-1 / apLocalTheta[1] * (con * pow(expr, apLocalTheta[2])
                                                                          * gsl_sf_bessel_Knu(apLocalTheta[2], expr))
                                                  - apLocalTheta[2] / apLocalTheta[1] * nu_expr - expr / apLocalTheta[1] * nu_expr_prime) *
                                                 sigma_square;
            }
            j0++;
        }
